...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    return find(data.cbegin(), data.cend(), value); // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int> add(const list<int> & data1, const list<int> & data2) noexcept {
    list<int> adds(data1.size());
    transform(data1.begin(), data1.end(), data2.begin(), adds.begin(), plus<int>());
    return adds;
}
