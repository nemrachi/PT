...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    ...
    for (string word : sentence) {
        if (translator.find(word) == translator.end()) { // not found
            translated.emplace_back("?");
        } else { // found
            translated.emplace_back(translator.at(word));
        }
    }
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    ...
    for (char c : mathExpression) {
        switch (c) {
            ...
            case ']':
            case '>':
                if (stck.empty() || stck.top() != parenthesis.at(c)) {
                    return false;
                }
                stck.pop();
                break;
...
list<int> createList(const list<int>::size_type length, const list<int>::size_type indexOfOne) {
    ...
    list<int> res(length, 0);
    auto it = res.begin();
    for (auto i = 0; i < indexOfOne; i++) {
        it++;
    }
    (*it) = 1;
...
void print2dVector(vector<vector<int>> vector) {
...
int main() {
    // queue<int> q = reverse({1, 2, 3, 4});
    // reverse(q);
    // printQueue(q);

    // vector<vector<int>> v = create2dVector(2, 5, 10);
    // print2dVector(v);

    // cout << connectWithSpace("ema", 123);

    // istringstream iss;
    // iss.str("10 10 30");
    // cout << sum(iss);

    // map<string, string> translator = {  {"read", "citat"},
    //                                     {"write", "pisat"},
    //                                     {"book", "kniha"},
    //                                     {"I", "ja"},
    //                                     {"you", "ty"},
    ...
    //                                     {"an", ""},
    //                                     {"the", ""}};
    // list<string> translated = translate(translator, { "He", "reads", "a", "book" });
